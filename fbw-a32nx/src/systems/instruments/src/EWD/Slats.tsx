...
import { Arinc429LocalVarConsumerSubject, Arinc429Word, Arinc429WordData } from '@flybywiresim/fbw-sdk';
...
export class Slats extends DisplayComponent<SlatsProps> {
  ...
  private readonly sfccToUse = MappedSubject.create(
    ([sfcc1Word, sfcc2Word]) => {
      console.log('sfccToUse');
      const sfcc1Valid = !sfcc1Word.isFailureWarning();
      const sfcc2Valid = !sfcc2Word.isFailureWarning();
      if (sfcc1Valid) {
        return 1;
      } else if (sfcc2Valid) {
        return 2;
  ...
  private readonly sfccSlatsFlapsStatus = MappedSubject.create(
    ([sfcc1Word, sfcc2Word]) => {
      if (this.sfccToUse.get() === 1) {
        return sfcc1Word;
      } else if (this.sfccToUse.get() === 2) {
        return sfcc2Word;
      } else {
        return Arinc429Word.empty() as Arinc429WordData;
      }
    },
    this.sfcc1SlatsFlapsStatus,
    this.sfcc2SlatsFlapsStatus,
  ...
  setSlatsPath(): void {
    const sfccSlatsPosition = this.sfccSlatsPosition.get();
  ...
  setFlapsPath(): void {
    const sfccFlapsPosition = this.sfccFlapsPosition.get();
  ...
  setSfccStatus(): void {
    const sfccSlatsFlaps = this.sfccSlatsFlapsStatus.get();
  ...
  onAfterRender(node: VNode): void {
    console.log('slats onAfterRender');
